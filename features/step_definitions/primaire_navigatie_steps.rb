...
Stel(/^als ondernemer bevind ik me op de home pagina en ben ingelogd$/) do
  visit '/'
  assert_equal "entrepreneur", @current_user.role 
end

Als(/^ik dan op de "(.*?)" link klik in het menu voor de ingelogde ondernemer$/) do |link|
  click_link @current_user.email
  click_link link
end
